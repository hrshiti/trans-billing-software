...
import { Wrench, User, MapPin, Phone, Loader2, CheckCircle2, ArrowRight } from 'lucide-react'
  ...
  return (
    <div className="animate-fadeIn" style={{ maxWidth: 480, margin: '0 auto' }}>
      <div className="auth-card-header" style={{ textAlign: 'center', marginBottom: 24 }}>
        ...
        <h2 className="auth-card-title" style={{ fontSize: '1.5rem', fontWeight: 800 }}>Setup Garage Business</h2>
        <p className="auth-card-subtitle">Register your garage workshop details to begin</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="card-flat" style={{ padding: 24, background: 'white', borderRadius: 24, border: '1px solid rgba(0,0,0,0.05)', boxShadow: '0 4px 20px rgba(0,0,0,0.03)' }}>
        ...
        <Field label="Garage Name" error={errors.businessName} required>
          <div className="input-group">
            <span className="input-prefix"><Wrench size={16} /></span>
            <input {...register('businessName', { required: 'Garage name is required' })} placeholder="e.g. Maruti Seva Sadan" className="form-input" />
          </div>
        </Field>
        ...
        <Field label="Workshop Address" error={errors.address} required>
          <div className="input-group" style={{ alignItems: 'flex-start' }}>
            ...
            <textarea {...register('address', { required: 'Address is required' })} placeholder="Workshop Address" className="form-input" style={{ minHeight: 80, paddingTop: 10 }} />
          </div>
        </Field>

        <button type="submit" className="btn btn-primary btn-lg btn-full" disabled={loading} style={{ marginTop: 8, background: '#7C3AED', borderColor: '#7C3AED' }}>
          {loading ? (
            <><Loader2 size={18} className="spin" /> Creating Workshop...</>
          ) : (
